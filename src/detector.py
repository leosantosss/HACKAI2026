...
class SegFormerDetector:
    ...
    def analyze_hazards(self, mask):
        """Analyze bottom center zone for general hazards (curbs/stairs). Uses EMA for smoothing."""
        h, w = mask.shape
        ...
        # Combined hazard pixels (stair + curb)
        hazard_pixels = np.isin(roi, self.nav_map['curb'] + self.nav_map['stair']).sum()
        
        roi_size = roi.size
        hazard_density = hazard_pixels / roi_size
        ...
        if not hasattr(self, 'hazard_history'):
            self.hazard_history = deque(maxlen=5)
            self.hazard_ema = 0.0
            
        alpha = config.APPROACH_EMA_ALPHA
            
        if hazard_density > min_density_threshold:
            # Update EMA
            self.hazard_ema = (alpha * hazard_density) + ((1 - alpha) * self.hazard_ema)
            self.hazard_history.append(self.hazard_ema)
            
            # Confirm it's consistent/growing before alerting
            if len(self.hazard_history) >= 2:
                is_significant = self.hazard_ema > (min_density_threshold * 1.2)
                if is_significant:
                    alerts.append({'type': 'hazard', 'confidence': float(self.hazard_ema)})
        else:
